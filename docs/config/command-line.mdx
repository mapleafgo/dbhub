...
<ParamField path="--config" type="string">
  ...
  When omitted, DBHub loads `./dbhub.toml` from the working directory if one exists. Only that directory is checked — parent directories are not searched. This is what lets a project's own `dbhub.toml` apply with no per-project setup, since MCP clients start DBHub in the project you opened. An explicit `--config` always wins, and a path that does not exist is an error rather than a fallback.
...
| `--config` | - | string | Path to TOML config file (default: `./dbhub.toml` in the working directory, if present) |
